...
def geometryMenu7_GrayVer():
    ...
    outH = int(scale * enterImageHight);
    outW = int(scale * enterImageWidth)
    print('[outH,outW]:',outH,outW)
    ## 출력이미지 메모리 할당
    outImage = createImageArray_GrayVer(outH, outW)
    ### 진짜 영상처리 알고리즘 ###
    for i in range(outH):
        for k in range(outW):
            printImage[i][k] = tmp[int(i / h)][int(k / h)]
    for j in range(h):
        for i in range(0, outH, h):
            for k in range(j, outW - 1, h):
                printImage[i][k] = int((printImage[i][k-1] + printImage[i][k+1]) / 2)
        for i in range(j, outH - 1, h):
            for k in range(outW):
                printImage[i][k] = int((printImage[i-1][k] + printImage[i+1][k]) / 2)
...
if __name__ == '__main__' :
    ...
    #[2-2] 메인메뉴 > 2.편집 메뉴
    editMenu = Menu(createMainMenu)
    createMainMenu.add_cascade(label="화소점 처리 메뉴", menu=editMenu)
    editMenu.add_command(label="(1)실행취소", command=rollbackMenu)

    #[2-3] 메인메뉴 > 2.화소점 처리 메뉴
    ...
    geometryMenu = Menu(createMainMenu)
    createMainMenu.add_cascade(label="기하학 처리", menu=geometryMenu)
    ...
    geometryMenu.add_command(label="영상 확대(양선형)", command=geometryMenu7_GrayVer)
    #
