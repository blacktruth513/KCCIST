...
def displayImageColor() :
    ...
    canvas.create_image((VY // 2, VX // 2), image=paper, state='normal')
...
def addColor() :
    ...
    second = end - start
    # status.configure(text=status.cget("text")+" "+"{0:.2f}".format(second))
...
def addColor_NP() :
    ...
    second = end-start
    # status.configure(text=status.cget("text") + "초 " + "{0:.2f}".format(second))
    status.configure(text="{0:.2f}".format(second) + "초 " + status.cget("text"))
