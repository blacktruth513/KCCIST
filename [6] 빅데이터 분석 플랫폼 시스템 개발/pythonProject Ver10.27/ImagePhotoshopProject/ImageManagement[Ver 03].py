...
def cvOut2outImage() : # OpenCV의 결과 --> OutImage 메모리에 넣기
    ...
    for _ in range(RGB):
        outImage.append(malloc(outH, outW))
    print('cvOutImage.ndim:',cvOutImage.ndim)
    #   3. 출력 메모리로 이미지 로딩작업
    for i in range(outH):
        for k in range(outW):
            if (cvOutImage.ndim == 2) : # 그레이, 흑백
                ...
                if k > int(outW / 2)-1:
                    count1 = k
                    print("k > int(outW / 2)-1 :[i:",i,",k",k," outW -k-1:", (k-(outW))*-1,"]")
                    # outImage[R][i][k] = cvInImage.item(i, outW -k-1)
                    # outImage[G][i][k] = cvInImage.item(i, outW -k-1)
                    # outImage[B][i][k] = cvInImage.item(i, outW -k-1)
            else :
                if k <= int(outW / 2 - 1):
                    count1 = k
                    # print("k<=inW :[i:",i,",k",k,"]")
                ...
                if k > 1 + int(outW / 2):
                    ...
                    outImage[R][i][k] = cvInImage.item(i, k - outW + 1)
                    outImage[G][i][k] = cvInImage.item(i, k - outW + 1)
                    outImage[B][i][k] = cvInImage.item(i, k - outW + 1)
