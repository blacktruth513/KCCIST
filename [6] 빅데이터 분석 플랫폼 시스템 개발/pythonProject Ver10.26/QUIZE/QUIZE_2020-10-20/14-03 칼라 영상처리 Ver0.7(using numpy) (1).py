...
def backImage():
    ...
    saveCvPhoto1 = np.zeros((outH, outW, 3), np.uint8)
    for i in range(outH) :
        for k in range(outW) :
            tup = tuple(([outImage[B][i][k],outImage[G][i][k],outImage[R][i][k]]))
            saveCvPhoto1[i,k] = tup

    saveCvPhoto2=cv2.imread(inImage,0)
    cvOutImage=cv2.inpaint(saveCvPhoto1, saveCvPhoto2,3,cv2.INPAINT_TELEA)

    displayImageColor()

def displayImageColor() :
    global window, canvas, paper, inImage, outImage, inH, inW, outH, outW, filename,status
    global cvInImage, cvOutImage
    if canvas != None :
        canvas.destroy()
    ##최대 화면 제한하기
    ...
    if outW <=VX or outH<=VY :
        ...
        if outW>outH :
            step = outW / VX
            VY = int(outH / outW *VX) ###########추가된 부분
        else :
            step = outH/ VY
            VX = int(outW / outH * VY) #########추가된 부분
    ...
    status.configure(text='이미지정보:' + str(outW) + 'x' + str(outH)+'      '+filename)
...
def savaExcel():
    ...
    if saveFp == '' or saveFp == None:
        return
    xlsName=saveFp.name
    #sheetName=os.path.basename(xlsName) #cat01_256.png
    wb= xlwt.Workbook()
...
def openExcel():
    ...
    wsList=workbook.sheets() #3장의 워크시트
...
def drawExcel():
    ...
    ws_color=wb.add_worksheet("COLOR")

    #섹의 크기 조절
    ws_R.set_column(0, outW-1, 1.0) #엑셀에서 0.34
    for i in range(outH):
        ws_R.set_row(i, 9.5) #엑셀에서 약 3.5


    ws_G.set_column(0, outW-1, 1.0) #엑셀에서 0.34
    for i in range(outH):
        ws_G.set_row(i, 9.5) #엑셀에서 약 3.5

    ws_B.set_column(0, outW-1, 1.0) #엑셀에서 0.34
    for i in range(outH):
        ws_B.set_row(i, 9.5) #엑셀에서 약 3.5

    ws_color.set_column(0, outW-1, 1.0) #엑셀에서 0.34
    for i in range(outH):
        ws_color.set_row(i, 9.5) #엑셀에서 약 3.5
    ...
    for i in range(outH):
        for k in range(outW):
            ...
            data=outImage[R][i][k]
            if data<=15:
                hexString='#'+('0'+hex(data)[2:])*3+'0000'

            else:
                hexString='#'+hex(data)[2:]*3+'0000'

            #셀 속성 변경
            cell_format=wb.add_format()
            cell_format.set_bg_color(hexString)
            ws_R.write(i,k,'',cell_format)

             ##Green시트
            data=outImage[G][i][k]
            if data<=15:
                hexString='#'+('000'+hex(data)[2:])*3+'00'

            else:
                hexString='#00'+hex(data)[2:]*3+'00'
            ...
            data=outImage[B][i][k]
            if data<=15:
                hexString='#'+('00000'+hex(data)[2:])*3
            else:
                hexString='#0000'+hex(data)[2:]*3
            #셀 속성 변경
            cell_format=wb.add_format()
            cell_format.set_bg_color(hexString)
            ws_G.write(i,k,'',cell_format)
##color시트
    for rgb in range(RGB):
        for i in range(outH):
            for k in range(outW):
                ##Color시트
                data=outImage[R][i][k]+outImage[G][i][k]+outImage[B][i][k]
                if data<=15:
                    hexString='#'+(hex(data)[2:])*3
                ...
                cell_format=wb.add_format()
                cell_format.set_bg_color(hexString)
                ws_color.write(i,k,'',cell_format)

    wb.close()
    print('ExcelArt .save...ok')
##openCV용 함수 모음
def cvOut2outImage(): #openCV의 결과 --> outImage 메모리에 넣기
    global window, canvas, paper, inImage, outImage, inH, inW, outH, outW, filename
    global cvInImage, cvOutImage, fileList
    ## 결과 메모리의 크기
    outH = cvInImage.shape[0]
    outW = cvInImage.shape[1]
    ## 입력이미지용 메모리 할당
    outImage = []
    for _ in range(RGB) :
        outImage.append(malloc(outH, outW))
...
def sharp_CV():
    ...
    cvOutImage=cv2.filter2D(cvInImage, -1, kernel)
    cvOutImage+=5
...
def guard_CV():
    ...
    cvOutImage = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)

    # 픽셀 강도의 중간값을 계산
    median_intensity = np.median(cvOutImage)

    # 중간 픽셀 강도에서 위아래 1 표준편차 떨어진 값을 임곗값으로 지정
    lower_threshold = int(max(0, (1.0 - 0.33) * median_intensity))
    upper_threshold = int(min(255, (1.0 + 0.33) * median_intensity))

    cvOutImage = cv2.Canny(cvOutImage, lower_threshold, upper_threshold)
    cvOut2outImage()
    ##################
    displayImageColor()
...
def mirror_CV():
    ...
    ##openCV용 영상처리##
    cvOutImage=cv2.filp(cvInImage,1)
    cvOut2outImage()
    ##################
    displayImageColor()
...
if __name__ == '__main__' :
    window = Tk()
    window.title('칼라 영상처리 Ver 0.6(with MySQL)')
    ...
    fileMenu.add_command(label="되돌리기(Back", command=backImage)
    ...
    pixelMenu.add_command(label="리버스(numpy)", command=reverseColor_NP)
    ...
    excelMenu.add_command(label="Excel에 저장", command=savaExcel)
    excelMenu.add_command(label="Excel에서 열기", command=openExcel)
    excelMenu.add_separator()
    excelMenu.add_command(label="Excel Art", command=drawExcel)


    openCVMenu=Menu(mainMenu)
    mainMenu.add_cascade(label="openCV", menu=openCVMenu)
    openCVMenu.add_command(label="그레이 스케일", command=gray_CV)
    openCVMenu.add_command(label="카툰 이미지", command=cartoon_CV)
    openCVMenu.add_command(label="선명하게", command=sharp_CV)
    openCVMenu.add_command(label="경계선 추출", command=guard_CV)
    openCVMenu.add_command(label="블러 처리", command=blur_CV)
    openCVMenu.add_command(label="좌우 대칭", command=mirror_CV)
    ######################
