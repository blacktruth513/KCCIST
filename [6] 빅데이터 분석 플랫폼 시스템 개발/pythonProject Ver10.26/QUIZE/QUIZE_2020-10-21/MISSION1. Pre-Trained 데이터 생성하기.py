...
clf = svm.SVC(gamma='auto').fit(study_data,study_lable)

# 6. 테스트 데이터를 통해 정확도를 확인한다.
testResult = clf.predict(test_data)
# print(testResult)

findLable = metrics.accuracy_score(testResult,test_lable)*100
print('테스트 데이터와 비교했을 때 정확도: %5.2f %%' % (findLable))
...
tempData = clf.predict([ [8,0.1,0.1,2,1,1,19,4,1,10.5,7] ])
print('이 와인은',tempData,'입니다. 단 %5.2f %% 의 확률입니다.' % (findLable*100))
